...
function EditDepartment() {
  ...
  useEffect(()=>{
    ...
    const fetchdep = async ()=>{
      try {
        const res = await axios.get(`https://employee-management-system-e8fx.vercel.app/api/department/${id}`)
        console.log(res);
  ...
  const handleSubmit = async (e)=>{
    e.preventDefault()
    try{
      const res = await axios.put(`https://employee-management-system-e8fx.vercel.app/api/department/${id}` ,{depname,desc})
      if(res.data.success) {
